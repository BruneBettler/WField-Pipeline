...
# Dataset type (e.g., 'led' or 'widefield')
DATASET_TYPE = 'led'

# Overwrite existing data? (Set to True to force reload raw frames)
OVERWRITE_RAW = False
...
def setup_paths():
    ...
    if nicole_path and os.path.exists(nicole_path):
        ...
        print(f"Added to sys.path: {nicole_path}")
    else:
...
def step1_load_raw(hdf5_file_path):
...
def step2_motion_correction(hdf5_file_path):
...
def step3_and_4_masking(hdf5_file_path, hdf5_creation_folder_path):
    print("\n--- Step 3 & 4: Masking ---")
...
def step5_hemo_correction(hdf5_file_path, path_to_full_mask):
    ...
    with h5py.File(hdf5_file_path, 'a') as f:
        ...
        # Define valid frames (Threshold 9000 based on previous findings)
        DARK_THRESHOLD = 9000
        ...
        # This prevents the High-Pass filter from 'ringing' at the dark edges
        for start, end in tqdm(ranges, desc="Processing Trials"):
            # A. Extract Trial Data (NumPy Array)
            # We copy to memory so hemo_correction sees a clean contiguous array
            trial_data = input_dset[start:end]
            
            # Check length - filters fail on tiny snippets
            if trial_data.shape[0] < 10:
                print(f"Skipping tiny fragment at {start}-{end}")
                continue

            # B. Run Hemo Correction on this trial ALONE
            # Returns numpy array (Time, H, W)
            try:
                # We disable verbose mostly to avoid cluttering specific trial times
                ...
                # C. Write back to correct location in HDF5
...
def step6_deltaF(hdf5_file_path):
    print("\n--- Step 6: Delta F / F ---")
    ...
    with h5py.File(hdf5_file_path, 'a') as f:
        ...
        # We use the validity of 'hemo_corrected' (NaNs) to determine which frames to include in F0
        ...
        for i in tqdm(range(0, n_frames, chunk_size), desc="Baseline F0"):
            ...
            # Load corrected chunk to check for NaNs
            dF_chunk = dF_dset[i:chunk_end]
            # A frame is valid if it is NOT all NaNs
            # (Checking one pixel per frame is enough if whole frame is NaN)
            middle_pixel = dF_chunk[:, dF_chunk.shape[1]//2, dF_chunk.shape[2]//2]
            valid_mask = ~np.isnan(middle_pixel)
            
            if np.any(valid_mask):
                # Load corresponding Raw Data (Blue)
...
def step7_visualization(hdf5_file_path, output_folder):
    print("\n--- Step 7: Visualization ---")
    
    with h5py.File(hdf5_file_path, 'r') as f:
        # Determine valid datasets to plot
        datasets_to_plot = []
        
        # Always try to plot raw frames (Channel 0)
        if 'raw_frames' in f[DATASET_TYPE]:
            datasets_to_plot.append('raw_frames')
            
        # Determine processed dataset
        ...
        for dset_name in datasets_to_plot:
            ...
            for i in tqdm(range(0, n_frames, chunk_size), desc=f"Trace ({dset_name})"):
                ...
                if dset.ndim == 3:
                    ...
                    # Assuming (T, C, H, W) - take channel 0 (Blue)
        ...
        # 3. SPECIAL: Comparison of Blue vs Violet (from Motion Corrected)
        if 'motion_corrected' in f[DATASET_TYPE]:
            ...
            for i in tqdm(range(0, n_frames, chunk_size), desc="Trace (Blue/Violet)"):
                chunk_end = min(i + chunk_size, n_frames)
                chunk = mc_dset[i:chunk_end] # Shape (T, 2, H, W)
                
                # Mean over H, W
                means = np.nanmean(chunk, axis=(2, 3)) # Shape (T, 2)
                blue_trace[i:chunk_end] = means[:, 0]
                violet_trace[i:chunk_end] = means[:, 1]

            # Normalize to compare shapes (Z-score or MinMax? Let's just plot raw to see correlation)
...
def main():
    try:
        ...
        step1_load_raw(hdf5_file_path)
        step2_motion_correction(hdf5_file_path)
        path_to_mask = step3_and_4_masking(hdf5_file_path, hdf5_creation_folder_path)
        step5_hemo_correction(hdf5_file_path, path_to_mask)
        step6_deltaF(hdf5_file_path)
        step7_visualization(hdf5_file_path, hdf5_creation_folder_path)
